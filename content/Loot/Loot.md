...
```mermaid  
graph TB  
    A[Player] --> B{Storage Nexus}  
    B -->|Slot Dasar| C[5 slot]  
    B -->|Upgrade| D[+3 slot per player tetap]  
    B -->|Sewa| E[5 DS/slot/sesi]  
    C --> F[Simpan material]  
    D --> G[Simpan gear legenda]  
    E --> H[Simpan consumable ekstra] 
```
